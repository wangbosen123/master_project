...
class correlation_system():
    def __init__(self, epochs, batch_num, batch_size, filename):
        ...
        self.geometry = load_model(f'model_weight/geometry-Lce+Limg-warp1.h5')
    ...
    def train_step(self, train_image, gt_image, points, label):
        with tf.GradientTape() as tape:
            ...
            total_loss = ce_loss + attention_loss + image_loss + style_loss
    ...
    def training(self, start_epoch):
        ...
        for epoch in range(start_epoch, self.epochs+1):
            ...
            for i in [0, 13, 26, 39, 52, 65, 78, 91, 104, 117]:
                self.plot_sample_train(epoch, num=i, shuffle_default=True)
                self.plot_sample_train(epoch, num=i, shuffle_default=False)
                self.plot_sample_test(epoch, num=i, shuffle_default=True)
                self.plot_sample_test(epoch, num=i, shuffle_default=False)
    ...
    def plot_sample_train(self, epoch, num, shuffle_default=False):
        ...
        plt.savefig(f'result/{self.filename}/{epoch}_shuffle_{shuffle_default}_{num}_train.jpg')
        plt.close()
    def plot_sample_test(self, epoch, num, shuffle_default=False):
        ...
        plt.savefig(f'result/{self.filename}/{epoch}_shuffle_{shuffle_default}_{num}_test.jpg')
        plt.close()
        if shuffle_default == False:
            with open(f'result/{self.filename}/Epoch-{epoch}-{num}-metrics-record.csv', 'w', newline='') as csvfile:
...
if __name__ == '__main__':
    ...
    correlation_system = correlation_system(130, 81, 150, filename='Lce+Limg-warp1')
    correlation_system.training(101)
    # for i in [0, 13, 26, 39, 52, 65, 78, 91, 104, 117]:
