...
def aligment():
    path1 = '/disk2/bosen/Datasets/AR_aligment_other/'
    source_path = []
    for id in os.listdir(path1):
        for filename in os.listdir(path1 + id):
            source_path.append(filename)

    path2 = '/disk2/bosen/Datasets/AR_original/'
    target_path = []
    for folder in os.listdir(path2):
        for filename in os.listdir(path2 + folder):
            if (filename in source_path) and (filename not in target_path):
                target_path.append(path2 + folder + '/' + filename)


    detector = dlib.get_frontal_face_detector()
    predictor = dlib.shape_predictor("/home/bosen/PycharmProjects/Datasets/shape_predictor_68_face_landmarks.dat")
    count = 0
    for num, filename in enumerate(target_path):
...
path1 = '/disk2/bosen/Datasets/AR_aligment_other/'
source_path = []
id_filename = [[] for i in range(90)]
for id in os.listdir(path1):
    for filename in os.listdir(path1 + id):
        id_filename[int(id[2:])-1].append(filename[0: 5])
        break
print(id_filename)
id_filename = np.array(id_filename).reshape(-1)
id_filename = list(id_filename)
print(id_filename)

id_path = [[] for i in range(90)]
path2 = '/disk2/bosen/Datasets/AR_original_alignment/'
for folder in os.listdir(path2):
    for filename in os.listdir(path2 + folder):
        if filename[0:5] in id_filename:
            image = cv2.imread(path2 + folder + '/' + filename, 0)
            if '-14-0' in filename or '-14-1' in filename or '-14-2' in filename or '-3-0' in filename or '-3-1' in filename or '-3-2' in filename:
                cv2.imwrite(f'/disk2/bosen/Datasets/AR_aligment_other_train/ID{id_filename.index(filename[0: 5])+1}/{filename}.jpg', image)
            if '-1-0' in filename or '-1-1' in filename or '-1-2' in filename or '-16-0' in filename or '-16-1' in filename or '-16-2' in filename:
                cv2.imwrite(f'/disk2/bosen/Datasets/AR_aligment_other_test/ID{id_filename.index(filename[0: 5]) + 1}/{filename}.jpg', image)
            # id_path[id_filename.index(filename[0: 5])].append(path2 + folder + '/' + filename)

# count = 0
# path = '/disk2/bosen/Datasets/AR_aligment_other_test/'
